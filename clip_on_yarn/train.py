...
def train_one_epoch(
    ...
    for i, batch in enumerate(trainloader):
        ...
        if (i_accum % 200 == 0) or (batch_count == num_batches_per_epoch):
            if ckpt_dir:
                others = {}
                if scaler:
                    others["scaler"] = scaler.state_dict()
                model_ckpt.save_ckpt(ckpt_dir, model, optimizer, epoch, **others)
                save_lora(ckpt_dir, model, epoch)

    if profiler:
        profiler.stop()


def save_lora(ckpt_dir: str, model, epoch):
    fs = fsspec.filesystem("hdfs")
    lora_dir = os.path.join(ckpt_dir, "lora")
    if not fs.exists(lora_dir):
        fs.mkdir(lora_dir)
    model_ckpt_path = os.path.join(lora_dir, f"epoch={epoch}")
    with TemporaryDirectory() as tmpdir:
        model.text_transformer.transformer.save_pretrained(tmpdir)
        fs.put(tmpdir, model_ckpt_path, recursive=True)
